...
n_max = 1000000
eps = 10**-10
test = True #тестовая или основная
...
if test == True:
    v = np.asarray(relax.top_relaxation_test(N, M, xs, ys, w, n_max, eps))
    n = v[M+1][:-N+2]
    v = v[:-1]
...
Метод верхней релаксации с параметром ω = {w},\n\
...
на сетке с половинным шагом использовано нулевое приближение.\n')
    ...
    table.field_names = ['\\', 'n', 'm', 'Итер.', 'ω', 'ε(N)', '||R(N)||', 'ε1']
    table.add_row(['Численное решение', f'{N}', f'{M}', f'{int(n[0])}', f'{w}', f'{n[1]:.11f}', f'{n[2]:.10f}', f'{eps1:.10f}'])
    ...
    v = np.asarray(relax.top_relaxation(N, M, xs, ys, w, n_max, eps))
    v2 = np.asarray(relax.top_relaxation(N*2, M*2, xs2, ys2, w2, n_max, eps))
    n = v[M+1][:-N+2]
    n2 = v2[M*2+1][:-N*2+2]
    v = v[:-1]
    v2 = v2[:-1]
...
метод верхней релаксации с параметром ω2 = {w2}, применены критерии остановки\n\
...
на сетке с половинным шагом использовано нулевое приближение.\n')
    ...
    table.field_names = ['\\', 'n', 'm', 'Итер.', 'ω', 'ε(N)', '||R(N)||', 'ε2']
    table.add_row(['Большой шаг', f'{N}', f'{M}', f'{int(n[0])}', f'{w}', f'{n[1]:.11f}', f'{n[2]:.10f}', f'{eps2:.10f}'])
    table.add_row(['Половинный шаг', f'{N*2}', f'{M*2}', f'{int(n2[0])}', f'{w2}', f'{n2[1]:.11f}', f'{n2[2]:.10f}',\
                   f'(x, y) = ({xm:.4f}, {ym:.4f})'])
    print(table, '\n')
